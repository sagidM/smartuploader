...
(function(){var $,DefaultOptions,DropZoneError,actions,empty,extract,ifWrongFileParams,imageAction,prettyJoin,uploadImageFile,uploadQuery;$=jQuery;DefaultOptions={url:null,action:null,multiUploading:!0,ifWrongFile:"show",maxFileSize:Number.POSITIVE_INFINITY,fileNameMatcher:/.*/,fileMimeTypeMatcher:/.*/,wrapperForInvalidFile:function(fileIndex){return "<p>File: \""+this.files[fileIndex].name+"\" doesn't support</p>'"},validateEach:function(fileIndex){return!0},validateAll:function(files){return files},uploadBegin:function(fileIndex,blob){},process:function(progress,fileIndex,blob){},uploadEnd:function(fileIndex,blob){},done:function(){},ajaxSettings:function(settings,fileIndex,blob){}};ifWrongFileParams=["nothing","error","show","ignore"];DropZoneError=function(message){return this.message=message};DropZoneError.prototype=new Error;extract=function(field,self,args){if(typeof field==="function"){return field.apply(self,args)}else{return field}};empty=function(){};$.fn.withDropZone=function(dropZone,options){var fileInput,key,promiseResult,uploadResult,value;if(this.attr("type")!=="file"){throw new DropZoneError("You should call this method only on input[type=file] and send dropZone as argument")}
...
uploadResult={load:null};promiseResult=new Promise(function(resolve){return uploadResult.load=resolve});fileInput=this.get(0);dropZone.on("dragenter",function(){return dropZone.addClass('hover')}).on("dragleave",function(e){return dropZone.removeClass('hover')}).on('dragover',function(e){return e.preventDefault()}).on("drop",function(e){e.preventDefault();fileInput.files=e.originalEvent.dataTransfer.files;return uploadImageFile(this,fileInput,promiseResult,fileInput.files,options)}).on("click",function(){return fileInput.click()});this.on("change",function(){return uploadImageFile(dropZone[0],this,promiseResult,this.files,options)});return uploadResult};uploadImageFile=function(dropZone,fileInput,promiseResult,files,options){var accept,action,actionOption,anyIsInvalid,blobs,dotPos,droppedFiles,ext,file,filenames,i,ignore,isValid,isValidFile,j,k,kids,len,mimeTypeMatcher,msize,multiUploading,name,nameMatcher,preview,previewContainer,progressBar,ref,uploadNext,uploadedFilesCount;dropZone.classList.remove('hover');dropZone.classList.remove('drop');dropZone.classList.remove('error');previewContainer=dropZone.getElementsByClassName("preview-container")[0];if(previewContainer){previewContainer.innerHTML=""}
...
wrapper.innerHTML='<div class="preview"></div>\n<div class="file-name"></div>\n<div class="file-uploader-progress-bar">\n  <div class="progress"></div>\n</div>';kids=wrapper.children;preview=kids[0];kids[1].textContent=file.name;progressBar=kids[2].children[0];uploadNext=function(blob){return promiseResult.then(function(){var formDataResult,process;process=function(progress){progressBar.style.width=100*progress.loaded/progress.total+"%";return options.process.call(fileInput,progress,fileIndex,blob)};if(multiUploading){formDataResult=options.formData.call(fileInput,fileIndex,blob,filenames[fileIndex]);return uploadQuery(options,formDataResult,filenames[fileIndex],fileInput,fileIndex,blob,process).done(function(){console.log("11",progressBar);wrapper.classList.remove("uploading");options.uploadEnd.call(fileInput,filenames[fileIndex],fileIndex,blob);if(++uploadedFilesCount===files.length){return options.done.call(fileInput,filenames)}})}else{blobs.push(blob);if(++uploadedFilesCount===files.length){formDataResult=options.formData.call(fileInput,blobs,filenames);return uploadQuery(options,formDataResult,filenames,fileInput,fileIndex,blob,process).done(function(){console.log("22",progressBar);options.uploadEnd.call(fileInput,filenames,fileIndex,blob);return options.done.call(fileInput,filenames)})}}})};filenames[fileIndex]=file.name;actionOption=extract(options.action,fileInput,[i]);if(actionOption){action=actions[actionOption.name];if(typeof action!=="function"){if(actionOption.name==null){throw new DropZoneError('Please, specify "name" in "action" block')}
...
if((convertTo.maxWidth!=null)||(convertTo.maxHeight!=null)){maxWidth=convertTo.maxWidth||this.width;maxHeight=convertTo.maxHeight||this.height;if(w>maxWidth){w=maxWidth;h*=w/this.width;if(h>maxHeight){h=maxHeight;w=this.width*h/this.height}}else if(h>maxHeight){h=maxHeight;w*=h/this.height;if(w>maxWidth){w=maxWidth;h=this.height*w/this.width}}
if((convertTo.width!=null)&&convertTo.width<maxWidth){w=convertTo.width}
if((convertTo.height!=null)&&convertTo.height<maxHeight){h=convertTo.height}}else if((convertTo.width!=null)&&convertTo.height){w=convertTo.width;h=convertTo.height}else{throw new DropZoneError("Not enough information about sizes")}
...
if(quality==1){quality=1.1}
return canvas.toBlob(blobCallback,convertTo.mimeType||file.type,quality)};reader=new FileReader;reader.onloadend=function(e){return img.src=e.target.result};return reader.readAsDataURL(file)};actions=$.fn.withDropZone.actions={image:imageAction}}).call(this)
